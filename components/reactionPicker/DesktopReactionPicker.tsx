  ...
  useEffect(() => {
    if(scrollAreaRef.current) {
      ...
      const handleWheel = (e: WheelEvent) => {
        e.preventDefault();
        scrollDiv.scrollTop += e.deltaY; // Adjust scroll position
      };
    ...
    <div className='relative isolate flex h-full flex-col overflow-hidden focus:outline-0'>
      <div className=' px-2 pt-2'>
        <Input
          ...
          placeholder='Search'
          className='
           h-8  border-transparent w-full'
          onKeyDown={handleKeyDown}
        />
      </div>

      <div ref={scrollAreaRef} className='flex-1 overflow-y-auto px-2 focus:outline-0' >
        <div
          style={{
          ...
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            if (rowData.type === 'header') {
              return (
                <h2
                  ...
                  className='  w-full whitespace-nowrap px-1.5 pb-1 pt-4 text-xs font-medium'
                  style={{
                    position: 'absolute',
                    top: 0,
                    left: 0,
            ...
            if (rowData.type === 'reactions') {
              return (
                ...
                >
                  {rowData.reactions.map((reaction: StandardReaction | SyncCustomReaction, columnIndex) => (
                    <button
                      title={reaction.name}
